...
Сервисы в сборке:
* Сервис с логикой (для упрощения авторизация и логика с командами, тасками в одном сервисе. Обычно делают в разных)
* Redis
* Prometheus
* Mysql
* RabbitMQ
...
# Запуск  
1) Для запуска требуется создать файл ```.env``` в корне директории, после скопировать в него содержимое файла ```.env.template```  
2) Запуск командой ```make run```  
